...
describe("update_activated_traps", {
  ...
  it("Expect error update_activated_traps()", {
    expect_error(update_activated_traps(active_traps, inactive_traps[1:15, ]), "🚨 Los IDs de los renglones 6 en IG_POSICION no están en el mapsource 🚨")
  })
  it("Expect two rows in message erro", {
    expect_error(update_activated_traps(active_traps, inactive_traps), "🚨 Los IDs de los renglones 6 y 16 en IG_POSICION no están en el mapsource 🚨")
  })
})
