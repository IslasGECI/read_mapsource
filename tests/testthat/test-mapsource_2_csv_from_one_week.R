...
describe("Read id, sunday, responsable name from last week", {
  it("read_id_sunday_and_responsable_from_last_week()", {
    ...
    expected <- readr::read_csv("/workdir/tests/data/expected_read_id_responsable_and_sunday.csv")
    expect_equal(obtained, expected)
  })
})
describe("copy status traps", {
  it("Example of 10MAR2023 but NA in status and responsable", {
    example_without_status <- readr::read_csv("/workdir/tests/data/example_IG_POSICION_TRAMPAS_10MAR2023_with_NA.csv")
    traps_status <- readr::read_csv("/workdir/tests/data/expected_read_id_responsable_and_sunday.csv")
    obtained <- example_without_status |>
      copy_trap_status(traps_status)
    expected <- readr::read_csv("/workdir/tests/data/expected_copy_trap_status.csv")
    expect_equal(obtained, expected)
  })
})
...
describe("Get position tramps with status from directory path", {
  it("get_current_position_tramps_from_directory()", {
    ...
    readr::write_csv(obtained, "/workdir/tests/data/expected_copy_trap_status_03JUL2022.csv")
    expected <- readr::read_csv("/workdir/tests/data/expected_copy_trap_status_03JUL2022.csv")
    expect_equal(obtained, expected)
  })
})
