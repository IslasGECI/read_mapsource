...
obtain_ig_posicion_mapsource_path <- function(root_path = "/workdir/data") {
  ...
  files <- comprehenr::to_vec(for (file in all_paths) if (stringr::str_detect(file, "IG_MAPSOURCE_TRAMPAS")) file)
...
obtain_ig_posicion_csv_path <- function(ig_posicion_mapsource_path) {
  stringr::str_replace_all(ig_posicion_mapsource_path, c("_MAPSOURCE_TRAMPAS" = "_POSICION_TRAMPAS", ".txt" = ".csv"))
}
